...
animate(covidanim, duration = 30, nframes = 200)
...
covid_bystate2.0 <- covid19 %>% 
  ...
  filter(est_pop_2018 >= 1,
         cases >= 1)

Covid19Map <- covid_bystate2.0 %>% 
  ggplot() +
  ...
  theme_map() +
  transition_reveal(date) +
  labs(title = "Cumulative COVID Cases per 10,000",
       subtitle = "Date: {frame_along}",
       fill = "") 

animate(Covid19Map, nframes = 200,end_pause = 10)

anim_save("covid19_anim.gif")
```

```{r}
knitr::include_graphics("covid19_anim.gif")
```

The advantage of the animated map is that you see when different hotspots became dangerous and not only where they are. The Dakotas in particular get brighter blue very quickly, faster than the nation as a whole.
...
[github](https://github.com/mdodge22/ProblemSet5/blob/master/DS_Week5.md)
